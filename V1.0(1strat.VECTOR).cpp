...
double galutinio_pazymio_skaiciavimas(vector<int>& kiekvieno_mok_pazymiai) {
	studentas Eil;
...
int main() {
	...
	for (int i = 0; i < nuskriaustukai.size(); i++) {
		nuskriaustuku_failas << nuskriaustukai.at(i).Vard << setw(20) << nuskriaustukai.at(i).Pav << setw(20) << nuskriaustukai.at(i).GALPAZ << endl;
	}
	...
	for (int j = 0; j < galvociai.size(); j++) {
			galvotuku_failas << galvociai.at(j).Vard << setw(20) << galvociai.at(j).Pav << setw(20) << galvociai.at(j).GALPAZ << endl;
		}
